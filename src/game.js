...
function makeRivals(){return rivalTemplates.map((r,i)=>({id:'r'+(i+1),name:r[0],style:r[1],focusGenre:r[2],target:r[3],strength:r[4],value:40000000+r[4]*900000,fame:r[4]/10,prestige:rnd(2,16),audience:300+r[4]*5,hits:0,nextStamp:stamp(2026,rnd(2,6))+i%2,nextGenre:Math.random()<.7?r[2]:genres[rnd(0,genres.length-1)]}))}
...
function processProjects(allowDialogs){
  for(const p of S.projects){
    ...
    // ── 本周指令 ──
    if(order==='rush'){step*=1.35;gain*=.7;team.forEach(e=>e.energy=clamp(e.energy-3,0,100));p.rush=(p.rush||0)+1;p.rushWeeks=(p.rushWeeks||0)+1}else p.rush=0;
    ...
    if(p.progress>=100){
      ...
      if(p.phase>=PHASES.length){p.ready=true;p.est=previewScore(p);p.rolled=rollScore(p);p.risk=scoreRisk(p);ensureOffers(p);addNews('成片完成',`《${p.name}》完成制作，发行商报价已经送达。`);if(allowDialogs){openRelease(p.id);break}}
...
function processRivals(){const now=stamp();S.rivals.forEach(r=>{if(now<r.nextStamp)return;const score=clamp(5.2+r.strength/30+(Math.random()-.5)*1.8,5.2,9.3),gross=(900000+score*620000)*(.75+Math.random()*.6),hit=score>=7.7;if(S.bet&&S.bet.rivalId===r.id&&S.bet.rivalScore==null){S.bet.rivalScore=score;resolveBet()}const own=Math.max(0,S.money+audienceTotal()*650+S.prestige*80000+S.ips.reduce((a,ip)=>a+ip.recognition*18000,0));
    const lead=own/Math.max(1,r.value);
    r.value=Math.max(3000000,(r.value*1.008+gross*.31-r.strength*11000)*(lead>1.35?1.014:1));r.fame=Math.max(1,r.fame+(hit?.7:-.15));r.prestige+=score>=8.4?2:0;r.audience+=Math.round(score*(hit?18:7));if(hit)r.hits++;S.audiences[r.target]=Math.max(20,S.audiences[r.target]-rnd(2,8));addNews('竞争对手',`${r.name}的${r.nextGenre}新作${hit?'成为档期热门':'表现平平'}，评分 ${score.toFixed(1)}。`);let ns=now+rnd(4,8);if(r.strength>=65&&Math.random()<.55)for(let k=0;k<3&&seasonAt(ns)[0]===1;k++)ns++;r.nextStamp=ns;r.nextGenre=Math.random()<.65?r.focusGenre:genres[rnd(0,genres.length-1)]})}
...
function prepareProjectDialog(){$('projectTitle').textContent=`筹备《${selectedScript.title}》`;$('filmName').value=selectedScript.title;$('filmScale').value=selectedScript.cost>300000?'cinema':'web';$('filmAudience').value=suggestedAudience(selectedScript);$('filmMarketing').value='100000';const ip=selectedScript.ipId?S.ips.find(x=>x.id===selectedScript.ipId):null;$('projectOriginHint').innerHTML=ip?`<b>${relationDefs[selectedScript.relation].name}</b> · IP知名度 ${Math.round(ip.recognition)}，当前疲劳 ${Math.round(ip.fatigue)}。高知名度提高开局，高疲劳会压低成片上限。`:`<b>原创项目</b> · 成功发行后将建立新IP，可继续开发续集、衍生作品和重启版本。`;$('crewPicks').innerHTML=S.staff.filter(e=>!e.status).sort((a,b)=>(b.role==='director')-(a.role==='director')||b.level-a.level).map((e,i)=>{
    const key=roles[e.role][1],keyName=({story:'故事',vision:'导演',performance:'表演',visual:'视听',management:'统筹'})[key];
    const rest=QUALITIES.map(q=>q[0]).concat('management').filter(k=>k!==key)
      .map(k=>`${({story:'故事',vision:'导演',performance:'表演',visual:'视听',management:'统筹'})[k]} ${e.stats[k]}`).join(' · ');
    const low=e.energy<40;
    return `<label class="pick"><input type="checkbox" value="${e.id}" ${i<5?'checked':''}><span class="pick-info">`
      +`<b>${safe(e.name)}</b> <span class="tag">${roles[e.role][0]} Lv.${e.level}</span>${e.star?` <span class="tag red">★</span>`:''}`
      +`<small><b class="key">${keyName} ${e.stats[key]}</b> · ${rest}</small>`
      +`<small>体力 <b class="${low?'warn':''}">${e.energy}</b>${low?'（状态不好，会拖质量）':''} · ${e.trait}${e.star?` · ${starTraits[e.starTrait]?.name||'明星'}`:''}</small>`
      +`</span></label>`;
  }).join('')||'<div class="empty">没有空闲主创。请先等待项目完成或招募新人。</div>';previewProject();openDialog('projectDialog')}
...
// 期望分：试映时能看出来的那部分。真实成片分见 rollScore()。
const SCALE_NEED={web:{studio:0,vision:42},cinema:{studio:1,vision:58},series:{studio:2,vision:70}};
function previewScore(p){
  const vals=Object.values(p.quality),craft=vals.reduce((a,b)=>a+b,0)/vals.length;
  const team=teamOf(p),coverage=new Set(team.map(e=>e.role));
  const roleBonus=['producer','director','writer','actor','camera','editor'].filter(r=>coverage.has(r)).length*.105;
  const facilityBonus=(S.facilities.screen?.22:0)+(S.facilities.sound?.09:0)+(S.facilities.edit?.09:0)+(S.facilities.casting?.05:0);
  const trendBonus=(p.script.genre===S.trend.name||p.script.theme===S.trend.name)?.42:0;
  const audBonus=audienceFit(p.script,p.target)?.2:-.12;
  const ip=S.ips.find(x=>x.id===p.ipId);
  const ipBonus=ip?Math.min(.4,ip.recognition/190)-Math.min(.8,ip.fatigue/85):0;
  const overwork=team.filter(e=>e.energy<25).length*.28+team.filter(e=>e.energy>=25&&e.energy<45).length*.07;
  const chemBonus=Math.min(.26,teamChemistry(team)*.065);
  const starBonus=Math.min(.28,team.reduce((a,e)=>a+(e.star?(e.starTrait==='versatile'?.14:.06):0),0));
  // 摊子铺得比公司本事大：场地不够、导演扛不住，都要还回来
  const need=SCALE_NEED[p.scale]||SCALE_NEED.web;
  const overreach=Math.max(0,need.studio-S.studio)*.3;
  const dir=team.filter(e=>e.role==='director').reduce((a,e)=>Math.max(a,e.stats.vision),0);
  const weakDirector=Math.min(.6,Math.max(0,need.vision-dir)*.017);
  return clamp(3.45+p.script.story/32+craft/27+roleBonus+facilityBonus+trendBonus+audBonus+ipBonus+chemBonus+starBonus
    -overwork-overreach-weakDirector, 2.6, scaleDefs[p.scale].cap);
}
// ── 成片的不确定性：同一套班子，拍出来是什么样没人先知道 ──
function gauss(){let u=0,v=0;while(!u)u=Math.random();while(!v)v=Math.random();return Math.sqrt(-2*Math.log(u))*Math.cos(2*Math.PI*v)}
function scoreRisk(p){
  const team=teamOf(p);
  let risk=.88;
  const lv=team.length?team.reduce((a,e)=>a+e.level,0)/team.length:1;
  risk-=Math.min(.34,(lv-1)*.09);                       // 老手手稳
  risk-=Math.min(.16,teamChemistry(team)*.05);          // 老搭档少翻车
  if(S.facilities.screen)risk-=.12;                     // 看片室能提前看出毛病
  if(S.facilities.edit)risk-=.05;
  risk-=Math.min(.16,(p.polish||0)*.08);                // 精剪
  risk+=team.filter(e=>e.energy<45).length*.07;         // 疲劳出事故
  risk+=Math.min(.24,(p.rushWeeks||0)*.03);             // 赶过的进度早晚要还
  risk+=p.scale==='series'?.13:p.scale==='cinema'?.07:0;// 摊子越大越难控
  if(S.ips.find(x=>x.id===p.ipId))risk-=.1;             // 有前作打底
  return clamp(risk,.26,1.4);
}
function rollScore(p){
  const est=previewScore(p),sd=scoreRisk(p);
  return clamp(+(est+gauss()*sd).toFixed(2),2.4,scaleDefs[p.scale].cap);
}
function riskLabel(sd){return sd<.45?'把握较大':sd<.72?'说不太准':sd<1?'心里没底':'纯属赌博'}
...
function renderRelease(){const p=projectById(selectedProjectId);if(!p)return;ensureOffers(p);if(!Number.isFinite(p.rolled)){p.rolled=rollScore(p);p.risk=scoreRisk(p)}const est=previewScore(p),sd=p.risk||scoreRisk(p),lo=clamp(est-sd,2.4,10),hi=clamp(est+sd,2.4,scaleDefs[p.scale].cap);const slot=releaseSlots()[0];$('releaseBody').innerHTML=`${sceneBanner('release-deal','《'+safe(p.name)+'》','三家发行商的报价都已经送到')}${qualityGrid(k=>Math.round(p.quality[k]||0))}<p>内部试映看下来，大概会落在 <b style="font-size:24px;color:var(--red)">${lo.toFixed(1)} ~ ${hi.toFixed(1)}</b> 分之间 · <b>${riskLabel(sd)}</b></p><p class="subtle">真正的分数要等观众看过才知道。想把范围收窄，靠老手、老搭档、看片室和精剪。核心受众 ${audienceDefs[p.target].name}。</p><h3>三份发行报价</h3><div class="deal-grid">${Object.keys(dealDefs).map((k,i)=>`<label class="deal"><input type="radio" name="releaseDeal" value="${k}" ${i===0?'checked':''} onchange="updateReleasePreview()"><span><b>${dealDefs[k].name}</b><small>${offerText(p,k,slot)}<br>${dealDefs[k].desc}</small></span></label>`).join('')}</div><div class="calendar-row"><div class="field"><label for="releaseSlot">选择首发档期</label><select id="releaseSlot" onchange="updateReleasePreview()">${releaseSlots().map(s=>{const se=seasonAt(s);return `<option value="${s}">${stampLabel(s)}${se[0]>1?` · ${se[1]} ×${se[0]}`:''}</option>`}).join('')}</select></div><div id="releasePreview" class="hint"></div></div><div class="hint">追加精剪真实消耗 2 周与 12 万元，最多 ${MAX_POLISH} 次（已用 ${p.polish}/${MAX_POLISH}）。等待档期期间主创会结束剧组工作，可以投入新项目。</div>`;$('polishBtn').disabled=p.polish>=MAX_POLISH||S.money<120000;requestAnimationFrame(updateReleasePreview)}
function openRelease(id){const p=projectById(id);if(!p||!p.ready)return;if(!Number.isFinite(p.rolled)){p.rolled=rollScore(p);p.risk=scoreRisk(p)}selectedProjectId=id;renderRelease();openDialog('releaseDialog')}
...
function postMortem(w){
  const d=w.diag||{};
  if(Number.isFinite(w.est)){
    if(w.score<w.est-.9)return '试映时都觉得能行，成片出来没接住——这行本来就没有保票。';
    if(w.score>w.est+.9)return '比试映时看好的还要好，剪出来的东西超过了素材本身。';
  }
